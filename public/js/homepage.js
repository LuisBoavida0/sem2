...
window.addEventListener('load', function(){
    ...
    for (let i = 0; i < parcelStatusEl.length; i++) {   //Go through each parcel
        if (title === 'Your parcels') datesEl[i].innerHTML = new Date(datesEl[i].innerHTML).toLocaleString()  //Format date
        else datesEl[i].innerHTML = Math.floor(Math.abs(new Date() - new Date(datesEl[i].innerHTML)) / 36e5)    //Convert the date into hours since now
